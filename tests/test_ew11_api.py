"""Tests for ew11_api.py — pure async, no HA dependency."""
...
# ── XML Parsing ────────────────────────────────────────────────────
class TestParseEW11Xml:
    """Tests for _parse_ew11_xml()."""

    def test_parses_uart_section(self):
        config = _parse_ew11_xml(SAMPLE_EW11_XML_OK)
    ...
    def test_empty_xml_returns_empty_config(self):
        config = _parse_ew11_xml("")
        assert config.uart == {}
        assert config.sock == {}
        assert config.sys == {}


# ── EW11Config properties ─────────────────────────────────────────
class TestEW11Config:
    """Tests for EW11Config dataclass properties."""

    def test_is_uart_ok_when_correct(self, good_ew11_config: EW11Config):
        assert good_ew11_config.is_uart_ok is True
        assert good_ew11_config.uart_issues == {}

    def test_is_uart_not_ok_when_wrong(self, bad_uart_config: EW11Config):
    ...
    def test_is_sock_ok_when_correct(self, good_ew11_config: EW11Config):
        assert good_ew11_config.is_sock_ok is True

    def test_is_sock_not_ok_when_wrong(self, bad_sock_config: EW11Config):
...
# ── EW11ValidationResult ──────────────────────────────────────────
class TestEW11ValidationResult:
    """Tests for EW11ValidationResult properties."""

    def test_all_ok_with_good_config(self, good_ew11_config: EW11Config):
    ...
    def test_uart_issues_proxy(self, bad_uart_config: EW11Config):
        result = EW11ValidationResult(
            reachable=True, auth_ok=True, config=bad_uart_config,
        )
        assert "gapTime Size" in result.uart_issues

    def test_sock_issues_proxy(self, bad_sock_config: EW11Config):
        result = EW11ValidationResult(
...
class TestPostCmd:
    ...
    async def test_successful_post(self):
        api = EW11Api(TEST_HOST, "admin", "admin")
    ...
    async def test_401_raises_auth_error(self):
        api = EW11Api(TEST_HOST, "admin", "wrong")
        with aioresponses() as m:
            m.post(f"http://{TEST_HOST}/cmd", status=401, body="Unauthorized")
            with pytest.raises(EW11AuthError, match="Authentication failed"):
    ...
    async def test_500_raises_api_error(self):
        ...
        with aioresponses() as m:
            ...
            with pytest.raises(EW11ApiError, match="HTTP 500"):
                await api._post_cmd({"CID": 10007})
...
class TestReadConfig:
    ...
    async def test_happy_path(self):
        ...
        with aioresponses() as m:
            ...
            m.get(f"http://{TEST_HOST}/EW11.xml", body=SAMPLE_EW11_XML_OK)
    ...
    async def test_xml_gen_failure(self):
        ...
        with aioresponses() as m:
            ...
            with pytest.raises(EW11ApiError, match="XML generation failed"):
    ...
    async def test_malformed_xml(self):
        ...
        with aioresponses() as m:
            ...
            with pytest.raises(EW11ApiError, match="no UART section"):
                await api.read_config()


# ── EW11Api.validate_config ───────────────────────────────────────
...
# ── EW11Api.configure_uart ────────────────────────────────────────
class TestConfigureUart:
    """Tests for configure_uart() using patch on read_config."""

    @pytest.mark.asyncio
    async def test_no_changes_needed(self, good_ew11_config: EW11Config):
        api = EW11Api(TEST_HOST, "admin", "admin")
        with patch.object(api, "read_config", return_value=good_ew11_config):
            result = await api.configure_uart()
            assert result.changed is False
            assert result.changed_fields == {}

    @pytest.mark.asyncio
    async def test_changes_applied(self, bad_uart_config, good_ew11_config):
        api = EW11Api(TEST_HOST, "admin", "admin")
        with (
            patch.object(
                api, "read_config", side_effect=[bad_uart_config, good_ew11_config]
            ),
            aioresponses() as m,
        ):
            m.post(f"http://{TEST_HOST}/cmd", payload={"RC": 0})
    ...
    async def test_sock_corruption_detected(self, bad_uart_config):
        ...
        corrupted_xml = SAMPLE_EW11_XML_OK.replace(
            "key='Protocol' value='TCP-SERVER'",
            "key='Protocol' value='UDP-SERVER'",
            1,  # Only replace SOCK, not UART
        )
        corrupted_config = _parse_ew11_xml(corrupted_xml)
        ...
        with (
            ...
            m.post(f"http://{TEST_HOST}/cmd", payload={"RC": 0})
            with pytest.raises(EW11SockCorruptedError, match="SOCK"):
    ...
    async def test_write_failure_raises(self, bad_uart_config):
        ...
        with (
            ...
            m.post(f"http://{TEST_HOST}/cmd", payload={"RC": 1})
            with pytest.raises(EW11ApiError, match="UART config write failed"):
                await api.configure_uart()
...
class TestRestart:
    ...
    async def test_restart_failure(self):
        ...
        with aioresponses() as m:
            m.post(f"http://{TEST_HOST}/cmd", payload={"RC": 1})
            with pytest.raises(EW11ApiError, match="Restart failed"):
                await api.restart()


# ── EW11Api.test_connection ───────────────────────────────────────
